...
def teacher_satrt_seance(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            seance = Seance(
                Identifiant_Professeur=request.user.teacher,
                Nom=form.cleaned_data['Nom'],
                HeureDebut=form.cleaned_data['HeureDebut'],
                HeureFin=form.cleaned_data['HeureFin'],
                Identifiant_Matiere=form.cleaned_data['matiere']
            )
            seance.save()
        ...
        form = SeanceForm(initial={
            'Identifiant_Professeur': user.id,
            'Nom': user.lname,
            'Prenom': user.fname ,
        })
...
def Student_attendance_view(request):
    seances = Seance.objects.filter(Identifiant_Professeur=request.user.teacher).prefetch_related('Identifiant_Matiere__Identifiant_Filiere__Identifiant_Annee')

    data = []
    for seance in seances:
        item = {
            'Identifiant_Seance': seance.Identifiant_Seance,
            'Identifiant_Professeur': seance.Identifiant_Professeur,
            'DateSeance': seance.DateSeance,
            'HeureDebut': seance.HeureDebut,
            'HeureFin': seance.HeureFin,
            'Identifiant_Matiere': seance.Identifiant_Matiere,
        }
        data.append(item)
